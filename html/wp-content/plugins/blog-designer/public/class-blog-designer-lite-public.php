...
class Blog_Designer_Lite_Public {
	...
	public function bd_front_stylesheet() {
		...
		wp_enqueue_script( 'wp-blog-designer-script', plugins_url( 'js/designer.js', __FILE__ ), null, '1.0', false );
	...
	public function bd_excerpt_length( $length ) {
		if ( '' != get_option( 'excerpt_length' ) ) { //phpcs:ignore
	...
	public function bd_views() {
		...
		if ( isset( $settings['template_authors'] ) && '' != $settings['template_authors'] ) { //phpcs:ignore
		...
		if ( '' != $display_sticky && 1 == $display_sticky ) { //phpcs:ignore
		...
		$class                = '';
		$alter_class          = '';
		$main_container_class = isset( $settings['main_container_class'] ) && '' != $settings['main_container_class'] ? esc_attr( $settings['main_container_class'] ) : ''; //phpcs:ignore
		if ( $loop->have_posts() ) {
				...
				<script type="text/javascript" id="flexslider_script">
					jQuery(document).ready(function () {
						...
						jQuery('.slider_template').flexslider({
							...
							<?php echo ( 1 == $display_slider_controls ) ? 'directionNav: true,' : 'directionNav: false,'; //phpcs:ignore ?>
							<?php echo ( 1 == $display_slider_navigation ) ? 'controlNav: true,' : 'controlNav: false,'; //phpcs:ignore ?>
							<?php echo ( 1 == $slider_autoplay ) ? 'slideshow: true,' : 'slideshow: false,'; //phpcs:ignore ?>
							<?php echo ( 1 == $slider_autoplay ) ? "slideshowSpeed:$slider_autoplay_intervals," : ''; //phpcs:ignore ?>
							<?php echo ( esc_attr( $slider_speed ) ) ? "animationSpeed: $slider_speed," : ''; //phpcs:ignore ?>
							prevText: "<?php echo $prev; //phpcs:ignore ?>",
							nextText: "<?php echo $next; //phpcs:ignore ?>",
							rtl: 
							<?php
							if ( is_rtl() ) {
								echo 1;
			...
			while ( have_posts() ) :
				...
				if ( 'classical' === $theme ) {
					...
					if ( 0 == get_option( 'template_alternativebackground' ) ) { //phpcs:ignore
				...
				echo apply_filters( 'bdads_do_show_ads', '', $settings, $i, $theme, $paged ); //phpcs:ignore
				$i++;
			endwhile;
			if ( 'timeline' === $theme ) {
				?>
		...
		if ( 'crayon_slider' !== $theme ) {
			...
			echo self::bd_pagination(); //phpcs:ignore
			echo '</div>';
		}
		if ( '' != $main_container_class ) { //phpcs:ignore
			echo '</div>';
	...
	public static function bd_get_content( $postid ) {
		...
		if ( 0 == get_option( 'rss_use_excerpt' ) ) { //phpcs:ignore
			$content = apply_filters( 'the_content', get_the_content( $postid ) );
		} elseif ( get_option( 'excerpt_length' ) > 0 ) {
			if ( 1 == $display_html_tags ) { //phpcs:ignore
				$text = get_the_content( $postid );
				if ( 0 == strpos( _x( 'words', 'Word count type. Do not translate!', 'blog-designer' ), 'characters' ) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) { //phpcs:ignore
				...
				if ( preg_match( '#(>|]|^)(([A-Z]|[a-z]|[0-9]|[\p{L}])(.*\R)*(\R)*.*)#m', $first_letter, $matches ) ) {
					...
					if ( ' ' == mb_substr( $content_change, 1, 1 ) ) { //phpcs:ignore
	 ...
	 */
	public function bp_close_tags( $html = '' ) {
