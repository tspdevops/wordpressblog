...
class Blog_Designer_Lite_Admin {
	...
	public function bd_session_start() {
		if ( -1 != version_compare( phpversion(), '7.0.0' ) ) { //phpcs:ignore
			if ( session_status() == PHP_SESSION_DISABLED ) { //phpcs:ignore
				session_start( array( 'read_and_close' => true ) );
			}
		} elseif ( -1 != version_compare( phpversion(), '5.4.0' ) ) { //phpcs:ignore
			if ( session_status() == PHP_SESSION_DISABLED ) { //phpcs:ignore
				session_start();
			}
		} else {
	...
	public function bd_save_settings() {
		if ( is_user_logged_in() && isset( $_POST['blog_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['blog_nonce'] ) ), 'blog_nonce_ac' ) ) {
			if ( isset( $_REQUEST['action'] ) && 'save' === $_REQUEST['action'] && isset( $_REQUEST['updated'] ) && 'true' === $_REQUEST['updated'] ) {
				...
				$settings = is_array( $settings ) ? $settings : unserialize( $settings ); //phpcs:ignore
	...
	public function bd_enqueue_color_picker( $hook_suffix ) {
		...
		if ( isset( $_GET['page'] ) && ( 'designer_settings' === $_GET['page'] || 'bd_getting_started' === $_GET['page'] || 'designer_welcome_page' === $_GET['page'] ) || 'plugins.php' === $hook_suffix ) { //phpcs:ignore
			global $wp_version;
			wp_enqueue_style( array( 'wp-color-picker', 'wp-jquery-ui-dialog' ) );
			if ( function_exists( 'wp_enqueue_code_editor' ) ) {
				wp_enqueue_code_editor( array( 'type' => 'text/css' ) );
			}
			wp_enqueue_script( 'my-script-handle', plugins_url( 'js/admin_script.js', __FILE__ ), array( 'wp-color-picker', 'jquery-ui-core', 'jquery-ui-dialog' ), '1.0', true );
			wp_localize_script(
				'my-script-handle',
				'bdlite_js',
				array(
			...
			wp_enqueue_script( 'my-chosen-handle', plugins_url( 'js/chosen.jquery.js', __FILE__ ), null, '1.8.2', false );
		}
	}
	/**
	...
	public function bd_admin_stylesheet() {
		...
		if ( ( isset( $_GET['page'] ) && ( 'designer_settings' === $_GET['page'] || 'bd_getting_started' === $_GET['page'] || 'designer_welcome_page' === $_GET['page'] ) ) || 'dashboard' === $screen->id || 'plugins' === $screen->id ) { //phpcs:ignore
			...
			if ( isset( $_GET['page'] ) && 'designer_settings' === $_GET['page'] ) { //phpcs:ignore
	...
	public function bd_get_page_link() {
		if ( isset( $_POST['page_id'] ) ) { //phpcs:ignore
	...
	public function bd_closed_bdboxes() {
		$closed = isset( $_POST['closed'] ) ? explode( ',', sanitize_text_field( wp_unslash( $_POST['closed'] ) ) ) : array(); //phpcs:ignore
		$page   = isset( $_POST['page'] ) ? sanitize_text_field( wp_unslash( $_POST['page'] ) ) : ''; //phpcs:ignore
		$user   = wp_get_current_user();
		if ( sanitize_key( $page ) != $page ) { //phpcs:ignore
			wp_die( 0 );
		}
		if ( ! $user ) {
			wp_die( -1 );
	...
	public function bd_template_search_result() {
		if ( isset( $_POST['temlate_name'] ) ) { //phpcs:ignore
			$template_name = sanitize_text_field( wp_unslash( $_POST['temlate_name'] ) ); //phpcs:ignore
		}
	...
	public function bd_footer() {
		if ( isset( $_GET['page'] ) && ( 'designer_settings' === $_GET['page'] || 'bd_getting_started' === $_GET['page'] ) ) { //phpcs:ignore
	...
	public function bd_submit_optin() {
		...
		$bd_submit_type = '';
		if ( isset( $_POST['email'] ) ) { //phpcs:ignore
			$bd_email = sanitize_email( wp_unslash( $_POST['email'] ) ); //phpcs:ignore
		} else {
			$bd_email = get_option( 'admin_url' );
		}
		if ( isset( $_POST['type'] ) ) { //phpcs:ignore
			$bd_submit_type = sanitize_text_field( wp_unslash( $_POST['type'] ) ); //phpcs:ignore
		}
		if ( 'submit' === $bd_submit_type ) {
			...
			$plugin_data['site_email']             = false != $bd_email ? $bd_email : get_option( 'admin_email' ); //phpcs:ignore
			...
			$plugin_data['plugin_name']              = 'Blog Designer';
			$plugin_data['plugin_slug']              = 'blog-designer';
			if ( isset( $_POST['selected_option_de'] ) ) { //phpcs:ignore
				$reason_id = sanitize_text_field( wp_unslash( $_POST['selected_option_de'] ) ); //phpcs:ignore
			}
			$plugin_data['deactivation_option'] = $reason_id;
			if ( isset( $_POST['selected_option_de_text'] ) ) { //phpcs:ignore
				$plugin_data['deactivation_option_text'] = sanitize_text_field( wp_unslash( $_POST['selected_option_de_text'] ) ); //phpcs:ignore
			}
			if ( 9 == $reason_id ) { //phpcs:ignore
				if ( isset( $_POST['selected_option_de_other'] ) ) { //phpcs:ignore
					$plugin_data['deactivation_option_text'] = sanitize_text_field( wp_unslash( $_POST['selected_option_de_other'] ) ); //phpcs:ignore
				}
			}
			$plugin_data['plugin_version']         = $current_version;
			$plugin_data['plugin_status']          = $status_type;
