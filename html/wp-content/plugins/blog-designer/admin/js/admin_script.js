...
jQuery('.bd-form-class .bd-setting-handle > li').click(function (event) {
    if (jQuery(this).hasClass('clickDisable')) {
        ...
        jQuery('#' + section).show();
        jQuery.post(ajaxurl, {
            action: 'bd_closed_bdboxes',closed: section,page: 'designer_settings'
        });
    }
});
...
function templateSearch($template_name) {
    ...
    jQuery.ajax({
        url: ajaxurl,method: 'POST',data: {'action': 'bd_template_search_result','temlate_name': $template_name,},
        success: function (response) {
