...
class Blog_Designer_Lite_Settings {
	...
	public static function bd_main_menu_function() {
		...
		<div class="wrap">
			...
			<?php
			$view_post_link = ( 0 != get_option( 'blog_page_display' ) ) ? '<span class="page_link"> <a target="_blank" href="' . esc_url( get_permalink( get_option( 'blog_page_display' ) ) ) . '"> ' . esc_html__( 'View Blog', 'blog-designer' ) . ' </a></span>' : ''; //phpcs:ignore
			if ( isset( $_REQUEST['bdRestoreDefault'] ) && isset( $_GET['updated'] ) && 'true' == sanitize_text_field( wp_unslash( $_GET['updated'] ) ) ) { //phpcs:ignore
				echo '<div class="updated" ><p>' . esc_html__( 'Blog Designer setting restored successfully.', 'blog-designer' ) . ' ' . $view_post_link . '</p></div>'; //phpcs:ignore
			} elseif ( isset( $_GET['updated'] ) && 'true' == sanitize_text_field( wp_unslash( $_GET['updated'] ) ) ) { //phpcs:ignore
				echo '<div class="updated" ><p>' . esc_html__( 'Blog Designer settings updated.', 'blog-designer' ) . ' ' . $view_post_link . '</p></div>'; //phpcs:ignore
			}
			...
			if ( isset( $_SESSION['success_msg'] ) ) {
				...
				<div class="updated is-dismissible notice settings-error">
					<?php
					echo '<p>' . $_SESSION['success_msg'] . '</p>'; //phpcs:ignore
			...
			<form method="post" action="?page=designer_settings&action=save&updated=true" class="bd-form-class">
				...
				<div class="wl-pages" >
					<div class="bd-settings-wrappers bd_poststuff">
						...
						<div class="bd-menu-setting">
							...
							<ul class="bd-setting-handle">
								...
								<?php if ( is_plugin_active( 'blog-designer-ads/blog-designer-ads.php' ) ) { ?>
									...
									?>
									<li data-show="bdpads" <?php echo $bdads_class; //phpcs:ignore ?>>
						...
						<div id="bdpgeneral" class="postbox postbox-with-fw-options" <?php echo $bdpgeneral_class_show; //phpcs:ignore ?>>
							<ul class="bd-settings">
								<li>
									...
									<div class="bd-left">
										<p class="bd-margin-bottom-50"><?php esc_html_e( 'Select your favorite layout from 10 free layouts.', 'blog-designer' ); ?> <b>
											<?php
											echo wp_kses(
										...
										<p class="bd-margin-bottom-30"><b><?php esc_html_e( 'Current Template:', 'blog-designer' ); ?></b> &nbsp;&nbsp;
											<span class="bd-template-name">
												...
												if ( isset( $settings['template_name'] ) ) {
													echo str_replace( '_', '-', esc_attr( $settings['template_name'] ) ) . ' '; //phpcs:ignore
										...
										<?php
										$template_name = '';
										if ( isset( $settings['template_name'] ) && '' != $settings['template_name'] ) { //phpcs:ignore
											...
											<div class="bd_selected_template_image">
												...
												?>
													...
													if ( isset( $settings['template_name'] ) && ! empty( $settings['template_name'] ) ) {
														...
														<?php
														if ( isset( $settings['template_name'] ) ) {
															echo str_replace( '_', '-', $settings['template_name'] ) . ' '; //phpcs:ignore
								...
								<li class="bd-caution">
									...
									<div class="bd-right">
										...
										<div class="select-cover">
											<?php
											echo wp_dropdown_pages( //phpcs:ignore
												array(
									...
									<div class="bd-right">
										...
										<select data-placeholder="<?php esc_attr_e( 'Choose Post Categories', 'blog-designer' ); ?>" class="chosen-select" multiple style="width:220px;" name="template_category[]" id="template_category">
											<?php foreach ( $categories as $category_obj ) : ?>
												...
												<?php
												if ( isset($settings['template_category']) && is_array($settings['template_category']) && in_array( $category_obj->term_id, $settings['template_category'] ) ) { //phpcs:ignore
									...
									<div class="bd-right">
										...
										<select data-placeholder="<?php esc_attr_e( 'Choose Post Tags', 'blog-designer' ); ?>" class="chosen-select" multiple style="width:220px;" name="template_tags[]" id="template_tags">
											<?php foreach ( $tags as $tag ) : ?>
												...
												<?php
												if ( isset($template_tags) && is_array($template_tags) && in_array( $tag->term_id, $template_tags ) ) { //phpcs:ignore
													echo 'selected="selected"';
												}
												?>
									...
									<div class="bd-right">
										...
										<select data-placeholder="<?php esc_attr_e( 'Choose Post Authors', 'blog-designer' ); ?>" class="chosen-select" multiple style="width:220px;" name="template_authors[]" id="template_authors">
											<?php foreach ( $blogusers as $user ) : ?>
												...
												<?php
												if ( isset($template_authors) && is_array($template_authors) && in_array( $user->ID, $template_authors ) ) { //phpcs:ignore
													echo 'selected="selected"';
												}
												?>
								...
								<li>
									...
									<div class="bd-right">
										...
										<textarea class="widefat textarea" name="custom_css" id="custom_css" placeholder=".class_name{ color:#ffffff }"><?php echo wp_unslash( get_option( 'custom_css' ) ); //phpcs:ignore ?></textarea>
						...
						<div id="bdpcontent" class="postbox postbox-with-fw-options" <?php echo $bdpcontent_class_show; //phpcs:ignore ?>>
							<ul class="bd-settings">
								<li>
									<div class="bd-left">
										<span class="bd-key-title">
								...
								<li class="read_more_on">
									...
									<div style="margin-bottom: 15px;">
										...
										<div class="bd-right">
											...
											$read_more_on = ( '' != $read_more_on ) ? $read_more_on : 2; //phpcs:ignore
											?>
						...
						if ( is_plugin_active( 'blog-designer-ads/blog-designer-ads.php' ) ) {
							...
							?>
							<div id="bdpads" class="postbox postbox-with-fw-options" <?php echo $bdads_class_show; //phpcs:ignore ?>>
								<div class="inside">
			...
			<div class="bd-admin-sidebar hidden">
				...
				<div class="bd-support">
					...
					<div class="help-wrapper">
						...
						<div style='display:block;margin-bottom:8px;'>
							<script src="//platform.linkedin.com/in.js" type="text/javascript"></script><?php //phpcs:ignore ?>
		...
		$response = wp_remote_get(
			...
			array(
				'body' => array(
					...
					'request' => serialize( (object) $args ), //phpcs:ignore
				),
			)
		);
		if ( ! is_wp_error( $response ) ) {
			$returned_object = unserialize( wp_remote_retrieve_body( $response ) ); //phpcs:ignore
			$plugins         = $returned_object->plugins;
		}
		$current_slug = 'blog-designer';
		if ( $plugins ) {
			foreach ( $plugins as $plugin ) {
				if ( $plugin->slug == $current_slug ) { //phpcs:ignore
